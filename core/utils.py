...
def random_perturb(feature_len, length, perturb_factor=0.1):
    r = np.linspace(0, feature_len, length + 1, dtype=np.uint16)
    perturbations = np.random.uniform(1 - perturb_factor, 1 + perturb_factor, size=len(r) - 1)
    r[1:] = np.cumsum(np.diff(r) * perturbations).astype(np.uint16) + r[0]
    r = np.clip(r, 0, feature_len-1) 
    return r
...
def save_test_record(test_info, file_path):
    fo = open(file_path, "w")
    fo.write("Step: {}\n".format(test_info["step"][-1]))
    fo.write("auc: {:.4f}\n".format(test_info["auc"][-1]))
    fo.write("ap: {:.4f}\n".format(test_info["ap"][-1]))
    fo.write("ac: {:.4f}\n".format(test_info["ac"][-1]))
    fo.write("far: {:.4f}\n".format(test_info["far"][-1]))
